...
def make_parser():
    ...
    net_group.add_argument(
        ...
        "times. The format of vmnet is '<bridge>/<ipaddr>/<netmask>', "
...
vmcreatelog = os.path.join(
    args.vmcreatelogdir, taskid + '_create_' + args.vmname + '.log')
...
try:
    subprocess.call(
        ['qemu-img', 'create', '-f',
            'raw', vmswapfile, str(args.vmswapsize) + "G"],
        stdout=open(os.devnull, 'wb'))
except Exception:
    logger.error(
...
try:
    ...
    logger.error(
        "Failed to mkswap: " + vmswapfile + ". " + str(sys.exc_info()[1]))
    cleanfailedcreate()
else:
...
try:
    ...
    for index, netitem in enumerate(args.vmnet):
        ...
        with open(if_file, 'w') as f:
            f.write(if_file_content.format(
                if_name, if_ip, if_prefixlen, ip_gateway))
